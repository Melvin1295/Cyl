...
empleadoControllers.controller('EmpleadoListadoCtrl', ['$scope', '$http', function ($scope, $http) {
    ...
    function empleados(){
        $http.get('http://localhost/rutas/php/?a=listar').then(function(r){
            $scope.model = r.data;
        });
    }
    function departamentos(){
        $http.get('http://localhost/rutas/php/?a=departamentos').then(function(r){
    ...
    $scope.retirar = function(id){
        if(confirm('Esta seguro de realizar esta acción?')){
            $http.get('http://localhost/rutas/php/?a=eliminar&id=' + id).then(function(r){
                empleados();
            });
        }
    }
    ...
    $scope.registrar = function(){
        ...
        $http.post('http://localhost/rutas/php/?a=registrar', model).then(function(r){
            empleados();
...
empleadoControllers.controller('EmpleadoVerCtrl', ['$scope', '$routeParams', '$http', function ($scope, $routeParams, $http) {
    $http.get('http://localhost/rutas/php/?a=obtener&id=' + $routeParams.id).then(function(r){
        $scope.model = r.data;
    });
}]);